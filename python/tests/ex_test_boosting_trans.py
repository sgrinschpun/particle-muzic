...
("pi+", 1, 2)])
def test_particle_conservations(particle_boosted, particle_transformation_output, particle_conservation_energy, particle_conservation_transverse_momentum, particle_conservation_momentum_components, particle_conservation_charge, particle, momentum,number,resolution):
    #number of expected particles
    assert len(particle_boosted.transformation.output) == number
    #energy conservation
    ...
    #transverse
    pt_in =  round(particle_conservation_transverse_momentum["pt_in"],resolution)
    pt_out =  round(particle_conservation_transverse_momentum["pt_out"],resolution)
    assert pt_in == pt_out
    #components
    p_in =  particle_conservation_momentum_components["p_in"]
    pt_out = particle_conservation_momentum_components["p_in"]
    for id, item in enumerate(p_in):
        assert round(item,resolution) == round(pt_out[id],resolution)
