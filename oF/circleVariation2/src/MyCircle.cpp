...
void MyCircle::setPosAmp(ofVec3f _pos_amp) {
    pos_amp = _pos_amp;
}

void MyCircle::setRotAmp(ofVec3f _rot_amp) {
    rot_amp = _rot_amp;
}

void MyCircle::setSpeedAmp(float _speed_amp) {
    speed_amp = _speed_amp;
}

void MyCircle::setColorMode(bool _col_mode) {
    col_mode = _col_mode;
}
